...
const ConfirmSelectionComponent = ({ closeConfirmationHandler }) => {
  ...
  useEffect(() => {
    ...
    const analyticObj = {
      ...
      event: aaEvents.REMOVE_NOMINEE_POPUP,
    };
  ...
  const getSelectedNomineesCount = () => {
    return (deletableNominees.length === selected.size && deletableNominees.length > 0)
  ...
  const onCloseHandler = () => {
    const analyticObj = {
      data: {
        _event: 'Remove_Nominee_Cancel',
        eventInfo: {
          name: 'Cancel',
          position: getSelectedNomineesCount(),
          component: 'Select Nominee',
        },
      },
      event: aaEvents.REMOVE_NOMINEE_CANCEL,
    };
    analyticEvents(analyticObj);
