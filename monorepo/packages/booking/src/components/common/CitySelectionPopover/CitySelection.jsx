...
const CitySelection = ({
  ...
  const {
    state: { airportsData, additional, main, pageType, popSearch },
  } = useAppContext();
  const { XPLORE } = Pages;
  let updatedLeg = [...legs];
  ...
  const {
    formState: { selectedSpecialFare, paxInfo, triptype },
  ...
  const onSearch = (e) => {
    ...
    const airportsListToSearch = airportsData.filter((row) => {
      if (citiesToExcludeWhileSearch.includes(row.stationCode)) {
        return false;
      }

      if (selectedSpecialFare) {
        const { internationalAllowed } = selectedSpecialFare;
        if (row.isInternational) {
          return internationalAllowed;
        }
      } else if (paxInfo.SRCT.Count > 0 && row.isInternational) {
        return false;
      }
