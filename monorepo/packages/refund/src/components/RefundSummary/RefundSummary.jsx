...
const RefundSummary = ({ aemData, apiData }) => {
  ...
  const getStatusList = (stepCode, refund) => {
    ...
    return stepLabels[stepCode]?.map((element, index) => {
      ...
      let iconClassName;
      if (className === REFUND_SUMMARY_CLASS.SUCCESS) {
        iconClassName = 'icon-tick';
      } else if (className === REFUND_SUMMARY_CLASS.FAILURE) {
        iconClassName = 'icon-close-simple';
      } else {
        iconClassName = element.icon;
      }
      return (
        <li className={className}>
          <Icon className={iconClassName} size="lg" />
                ...
                <div className="refundwrapper mb-5">
                  <ul className="sessions">
                    <li className={REFUND_SUMMARY_CLASS.SUCCESS}>
                      <Icon className="icon-tick" size="lg" />
