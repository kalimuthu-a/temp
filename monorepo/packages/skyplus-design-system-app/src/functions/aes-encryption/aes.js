 ...
 */
export const Cipher = (input, w) => { // main Cipher function [Â§5.1]
  ...
  let state = [[], [], [], []]; // initialise 4xNb byte-array 'state' with input [Â§3.4]
  ...
  const output = new Array(4 * Nb); // convert state to 1-d array before returning [Â§3.4]
...
function MixColumns(s, Nb) { // combine bytes of each col of state S [Â§5.1.3]
  for (let c = 0; c < 4; c++) {
    ...
    const b = new Array(4); // 'b' is aâ€¢{02} in GF(2^8)
    for (let i = 0; i < 4; i++) {
      a[i] = s[i][c];
...
export const KeyExpansion = (key) => { // generate Key Schedule (byte-array Nr+1 x Nb) from Key [Â§5.2]
  const Nb = 4; // block size (in words): no of columns in state (fixed at 4 for AES)
